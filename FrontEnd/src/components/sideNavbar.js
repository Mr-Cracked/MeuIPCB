...
export default function SideNavbar() {
  ...
  const menuCalendarioRef = useRef(null);
  const menuEscolaRef = useRef(null);
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      const clickedOutsideCalendario =
          calendarioRef.current &&
          !calendarioRef.current.contains(event.target) &&
          menuCalendarioRef.current &&
          !menuCalendarioRef.current.contains(event.target);

      const clickedOutsideEscola =
          escolaRef.current &&
          !escolaRef.current.contains(event.target) &&
          menuEscolaRef.current &&
          !menuEscolaRef.current.contains(event.target);

      if (clickedOutsideCalendario) setCalendarioAberto(false);
      if (clickedOutsideEscola) setEscolaAberto(false);
    }
      ...
      <>
        <div className="sidebar open">
          ...
          <ul className="nav-list">
            ...
            <li className={isAtivo("/ai") ? "active" : ""} onClick={() => navigate("/ai")}>
