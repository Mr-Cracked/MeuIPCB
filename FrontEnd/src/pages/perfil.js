...
export function Perfil() {
  ...
  async function fetchTarefas() {
    try {
      ...
      setTarefas(data.filter(t => !t.concluida));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    async function fetchUserProfile() {
      try {
        ...
        else if (status === 403) navigate("/anuncioscoord", { replace: true });
    ...
    async function fetchAnuncioRecente() {
      try {
        const { data } = await axios.get("http://localhost:3000/api/anuncio/veraluno", {
          withCredentials: true
        });
  ...
  const getCorPrioridade = (p = "") => {
    const pr = (p || "").toString().toLowerCase().trim();
  ...
  const handleAtualizacao = (tAtualizada) => {
    if (!tAtualizada || !tAtualizada._id) {
      console.error("Tarefa atualizada é inválida:", tAtualizada);
      return;
    }
    console.log("Tarefa atualizada:", tAtualizada);
